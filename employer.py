...
'''def cmd_select(selected):
    global selected_applicant
    selected_applicant = users_db['Employees'].find_one(ID=list_Employees.get(curselection()[0])[0][0])'''
...
def view_detail():
    ...
    detail_window.geometry('300x380')
    ...
    resume.pack(pady=10, padx=10)
    yes_button.pack(side=LEFT, pady=8, padx=5)
    no_button.pack(side=LEFT, pady=8, padx=5)
    frame_buttons.pack()
...
for reply in job_db['Replies']:
        ...
        if job['CompanyID'] == current_user['ID']:
...
apply_button.pack(pady=8, padx=5)
...
# 更新按鈕
save_button = Button(tab2, text="張貼職缺", command=job_update)
...
intro_entry.grid(row=1, column=1, sticky=E + W)
intro_entry.insert(0, current_user['companyIntro'])

# 更多空間
rules_prompt = Label (tab3, text="公司制度").grid(row=2, column=0, sticky=N + W)
rules_entry = Entry(tab3)
rules_entry.grid(row=2, column=1 , sticky=N + W + E + S)
rules_entry.insert(0, current_user['companyDes'])
save_button = Button(tab3, text="更新內容", command=lambda:company_update())
